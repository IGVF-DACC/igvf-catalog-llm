...
def get_url_prefix(config: Config) -> str:
    ...
    return f'catalog-llm-{config.branch}'
...
class Frontend(Construct):
    ...
    def _define_docker_assets(self) -> None:
        self.application_image = ContainerImage.from_asset(
            '../',
            file='docker/flask/Dockerfile.prod',
        )
    ...
    def _define_fargate_service(self) -> None:
        ...
        self.fargate_service = ApplicationLoadBalancedFargateService(
            self,
            'Fargate',
            service_name='Catalog-LLM-Service',
    ...
    def _add_application_container_to_task(self) -> None:
        ...
        self.fargate_service.task_definition.add_container(
            ...
            secrets={
                'CATALOG_USERNAME': Secret.from_secrets_manager(catalog_llm_secret, 'CATALOG_USERNAME'),
                'CATALOG_PASSWORD': Secret.from_secrets_manager(catalog_llm_secret, 'CATALOG_PASSWORD'),
                'OPENAI_API_KEY': Secret.from_secrets_manager(catalog_llm_secret, 'OPENAI_API_KEY'),
            },
